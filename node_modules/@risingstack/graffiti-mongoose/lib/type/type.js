...
var GraphQLViewer = new _type.GraphQLObjectType({
  name: 'Viewer',
...
function getOrderByType(_ref, fields) {
  ...
  if (!orderByTypes[name]) {
    // save new enum
    orderByTypes[name] = new _type.GraphQLEnumType({
      name: 'orderBy' + name,
      values: (0, _lodash.reduce)(fields, function (values, field) {
        if (field.type instanceof _type.GraphQLScalarType) {
...
function getArguments(type) {
  ...
  return (0, _lodash.reduce)(fields, function (args, field) {
    ...
    if (field.type instanceof _type.GraphQLNonNull && field.name !== 'id') {
...
function getType(graffitiModels, _ref2) {
  ...
  var graphQLTypeFields = (0, _lodash.reduce)(fields, function (graphQLFields, _ref3, key) {
    ...
    if (type === 'Array') {
      if (subtype === 'Object') {
        ...
        graphQLField.type = new _type.GraphQLList(getType(graffitiModels, { name: nestedObjectName, description: description, fields: _fields }, newPath, rootType));
      } else {
        graphQLField.type = new _type.GraphQLList(stringToGraphQLType(subtype));
        if (reference) {
          resolveReference[rootType.name][name] = {
            ...
            resolve: (0, _utils.addHooks)(function (rootValue, args, info) {
              ...
              return (0, _query.connectionFromModel)(graffitiModels[reference], args, info);
            }, hooks)
          };
        }
      }
    ...
    if (reference && (graphQLField.type === _type.GraphQLID || graphQLField.type === new _type.GraphQLNonNull(_type.GraphQLID))) {
    ...
    if (nonNull && graphQLField.type) {
      graphQLField.type = new _type.GraphQLNonNull(graphQLField.type);
    }
  ...
  // Define type
  var GraphQLObjectTypeDefinition = new _type.GraphQLObjectType(graphQLType);
    ...
    if (type) {
      var typeFields = (0, _lodash.reduce)(fields, function (typeFields, field, fieldName) {
        ...
        while (path.length > 0) {
          ...
          if (parent[segment]) {
            if (parent[segment].type instanceof _type.GraphQLObjectType) {
              parent = parent[segment].type.getFields();
            } else if (parent[segment].type instanceof _type.GraphQLList && parent[segment].type.ofType instanceof _type.GraphQLObjectType) {
